...
export function toEntity<TEntity extends Entity>(
  entityMetadata: EntityMetadata,
  webApiRetrieveResponse: { [k: string]: any }
): TEntity {
...
function getAttribute(
  ...
  const attributeMetadata = getAttributeMetadata(
    entityMetadata,
    attributeName,
    false
  );
  ...
  switch (attributeMetadata.attributeType) {
    ...
    case 'owner':
      const lookupValue: Xrm.LookupValue = {
        ...
        name:
          webApiRetrieveResponse[
            `${name}@OData.Community.Display.V1.FormattedValue`
          ],
        entityType:
          webApiRetrieveResponse[
            `${name}@Microsoft.Dynamics.CRM.lookuplogicalname`
          ],
      };
    ...
    case 'datetime':
      return {
        name: attributeName,
        value: new Date(webApiRetrieveResponse[name]),
      };
